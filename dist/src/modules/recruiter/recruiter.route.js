...
export function createRecruiterRoutes(jobCollection, applicationCollection, recruiterRequestCollection, userCollection) {
    const router = Router();
    router.post('/apply', verifyToken, async (req, res) => {
        try {
            ...
            if (existing)
                return sendError(res, 'Request already exists', 409);
    ...
    router.post('/jobs', verifyToken, requireRole('recruiter', 'admin'), async (req, res) => {
        try {
            const userId = req.user?.sub;
            if (!userId)
                return sendError(res, 'Unauthorized', 401);
            const { title, companyName, companyLogo, category, jobType, location, salaryMin, salaryMax, deadline, shortDescription, fullDescription, requirements, benefits, recruiterId, recruiterName, recruiterImage, recruiterEmail, } = req.body;
    ...
    router.get('/jobs', verifyToken, requireRole('recruiter', 'admin'), async (req, res) => {
        try {
            const userId = req.user?.sub;
            if (!userId)
                return sendError(res, 'Unauthorized', 401);
    ...
    router.get('/jobs/:id', verifyToken, requireRole('recruiter', 'admin'), async (req, res) => {
        try {
            const userId = req.user?.sub;
    ...
    router.patch('/jobs/:id', verifyToken, requireRole('recruiter', 'admin'), async (req, res) => {
        try {
            const userId = req.user?.sub;
            const id = String(req.params.id);
            if (!ObjectId.isValid(id)) {
            ...
            const result = await jobCollection.updateOne({ _id: new ObjectId(id), postedBy: userId }, { $set: updateData });
    ...
    router.delete('/jobs/:id', verifyToken, requireRole('recruiter', 'admin'), async (req, res) => {
        try {
            const userId = req.user?.sub;
            const id = String(req.params.id);
            if (!ObjectId.isValid(id)) {
    ...
    router.get('/jobs/:jobId/applicants', verifyToken, requireRole('recruiter', 'admin'), async (req, res) => {
        try {
            ...
            const applicants = await applicationCollection
                ...
                .sort({ createdAt: -1 })
                .toArray();
            const applicantsWithUser = await Promise.all(applicants.map(async (app) => {
                let user = null;
                if (app.userId && ObjectId.isValid(app.userId)) {
                    user = await userCollection.findOne({ _id: new ObjectId(app.userId) }, { projection: { name: 1, email: 1, image: 1 } });
                }
                return { ...app, user };
            }));
    ...
    router.patch('/applications/:appId/status', verifyToken, requireRole('recruiter', 'admin'), async (req, res) => {
        try {
            ...
            const result = await applicationCollection.updateOne({ _id: new ObjectId(appId) }, { $set: updateData });
            if (result.matchedCount === 0) {
    ...
    router.get('/analytics/overview', verifyToken, requireRole('recruiter', 'admin'), async (req, res) => {
        try {
            const userId = req.user?.sub;
