...
export function createPaymentRoutes(userCollection, applicationCollection, jobCollection, plansCollection, subscriptionsCollection) {
    const router = Router();
    ...
    router.get('/subscription', verifyToken, async (req, res) => {
        try {
            ...
            const planId = user.plan || 'free_seeker';
    ...
    router.get('/my-subscription', verifyToken, async (req, res) => {
        try {
            ...
            if (!userId)
                return sendError(res, 'Unauthorized', 401);
            const subscription = await subscriptionsCollection.findOne({ userId, status: 'active' }, { sort: { createdAt: -1 } });
    ...
    router.post('/create-checkout', verifyToken, async (req, res) => {
        try {
            ...
            const planDoc = await plansCollection.findOne({ id: planId });
            if (!planDoc)
                return sendError(res, 'Plan not found', 404);
            if (planDoc.isFree)
                return sendError(res, 'Cannot checkout for free plan', 400);
            const user = await userCollection.findOne({ _id: new ObjectId(userId) });
            if (!user)
                return sendError(res, 'User not found', 404);
            if (user.plan === planId) {
    ...
    router.post('/confirm', verifyToken, async (req, res) => {
        try {
            ...
            if (!sessionId)
                return sendError(res, 'sessionId required', 400);
    ...
    router.post('/cancel', verifyToken, async (req, res) => {
        try {
            ...
            const fallbackPlan = user.role === 'recruiter' ? 'recruiter_free' : 'free_seeker';
            await userCollection.updateOne({ _id: new ObjectId(userId) }, { $set: { plan: fallbackPlan, stripeSubscriptionId: null, updatedAt: new Date() } });
            // Update subscription record
            await subscriptionsCollection.updateOne({ userId, stripeSubscriptionId: user.stripeSubscriptionId }, { $set: { status: 'cancelled', updatedAt: new Date() } });
